...
// Before you get started writing code that passes the tests, be sure 
// to do `npm run start` and open http://localhost:8000 so you can 
// also see the results of your work in the web browser. This will 
// make it easier to understand what the tests are asking for. 
...
// Remember that you can open multiple terminal 
// emulator windows or mutliple tabs within your terminal 
// emulator – this will let you run the web server at the 
// same time that you run the tests using Mocha.

describe('Slice 1: Clicking & Incrementing Coffee', function() {
  beforeEach('reset the fake DOM', function() {
    resetJSDOM();
  });
  // First, we're having you write a function that will 
  // run every time the coffee icon is clicked. You don't need 
  // to attach this as an event listener, as we've done that for 
  // you near the bottom of `script.js`.
...
// you should be able to open the page up in your browser 
...
// Try opening up your browser's console. 
// On a Mac in Chrome, the shortcut is cmd + 
// option + J. Here, you have access to all of 
// the functions defined in the global scope.

// Try running `updateCoffeeView(5000)`. What do you see? Why?

// Now try runnning `data`. You'll see 
// the data object printed to the console, 
// which looks something like: `{coffee: 0, totalCPS: 0, 
// producers: Array(12)}`. You can click the arrow to expand this object.

// To debug, you can manually manipulate 
// variables to see how the app responds. Try 
// `data.coffee = 5000`, then try running `data` again. 
// The data has changed, but what we see on screen hasn't 
// changed. Why? Finally, try running `clickCoffee(data);`. 
...
describe('Slice 2: Unlocking & Rendering Producers', function() {
  ...
  // This far into Slice 2, you've defined a function 
  // that renders the producers to the screen, with the 
  // help of some other functions. What producers are rendered 
  // depends, of course, on how much coffee the player has accumulated.

  // While it might be passing the tests, nothing in our code 
  // yet calls the renderProducers function. How can we test it in the browser?

  // Try running `renderProducers(data)` in the browser 
  // console. This might show you some producers, depending 
  // on how much coffee you have. You can click a bunch more 
  // times and run the function again to test it out, or you can 
  // just set `data.coffee` to a big number before running 
  // `renderProducers(data)`. Try that to see if the function works!

  // How is our code actually going to run this function? 
  // That's what the next test, the last one in Slice 2, addresses; 
  ...
  describe('The clickCoffee function', function() {
    ...
    // Set up some fake data. Note that we're just below the threshold 
    // to unlock and render producer_B.
    let data;
    beforeEach('initialize some fake data', function() {
      data = {
        coffee: 99,
...
// That's the end of Slice 2! In the browser, you should now 
// have an interactive app that lets you click the coffee mug to 
// get coffee, and renders producers to the screen as they become unlocked.

// Next, we'll wire up the 'buy' buttons on the proucers 
// and set up a 'tick' function that 'runs' the producers, 
...
describe('Slice 3: Buying Producers & Tick', function() {
  ...
  describe('The buyButtonClick function', function() {
    ...
    // Notice that at the bottom of `script.js` 
    // we attach an event listener that calls `buyButtonClick` 
    // not just to a buy button but to the entire producer container. 
    // Here, we test that you filter clicks so that the function pays 
    // attention only to clicks on buy buttons.
